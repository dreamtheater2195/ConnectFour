...
describe Board do
  ...
  describe '#new' do
    it "create an instance of Board" do
      expect(gameboard).to be_instance_of(Board)
    end
  end

  describe "#clear board" do
    it "make board have #{Board::COLS} columns" do
      expect(gameboard.board.length).to eq(Board::COLS)
    end

    it "make columns of length #{Board::ROWS}" do
      gameboard.board.each do |col|
        expect(col.length).to eq(Board::ROWS)
      end
    end

    it "make all cells of board blank" do
      gameboard.board.each do |col|
        col.each do |cell|
